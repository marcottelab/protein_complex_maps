...
def main():
    ...
    #CDM Added. Note that clean ppis are generated from these sets
    test_list_filename = '.'.join(args.complex_filename.split('.')[:-1]) + '.all_test.txt'
    train_list_filename = '.'.join(args.complex_filename.split('.')[:-1]) + '.all_train.txt' 
...
def split_complexes(complexes, size_threshold=None, threshold_fraction=None, remove_large_complexes=False, input_complexes=None): 
    ...
    return test_list_clean, train_list_clean, test_ppis_clean, train_ppis_clean, neg_test_ppis_clean, neg_train_ppis_clean, test_list, train_list
