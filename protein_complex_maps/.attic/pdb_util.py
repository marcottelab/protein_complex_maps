...
class PISA_Interfaces(object):
    ...
    def __init__(self, pisa_filename, pdbid=None):
        ...
        print self.interfaces_area

        if self.pdbid != None:
            #self.chain2acc = pu.get_pdb_protein_ids(self.pdbid, reversible=True)
            self.chain2acc = pu.get_pdb_protein_ids(self.pdbid)
            self.acc2chain = pu.get_pdb_protein_ids_chainid_map(self.pdbid)
            print self.chain2acc
            print self.acc2chain
    ...
    def map_orthologs(self, base_species):
        ...
        self.acc2base_species[base_species] = pu.get_ortholog( [x for x in self.acc2chain.keys() if x != None], species1=base_species, reversible=True)
        print self.acc2base_species

    def surface_area_set(self, chainA_set, chainB_set):
        sa = None
        for chainA in chainA_set:
            for chainB in chainB_set:
                tmp_chainA = chainA
                tmp_chainB = chainB
                if tmp_chainA > tmp_chainB:
                    tmp_chainA, tmp_chainB = tmp_chainB, tmp_chainA

                print "surface_area_set: %s %s" % (tmp_chainA, tmp_chainB)

                try:
                    sa = self.interfaces_area[(tmp_chainA, tmp_chainB)]
                except KeyError:
                    continue
    ...
    def surface_area_by_acc(self, acc1, acc2, base_species=None):
        ...
        if base_species != None:
            try:
                ...
                c1_set = self.acc2chain[self.acc2base_species[base_species][acc1]]
                c2_set= self.acc2chain[self.acc2base_species[base_species][acc2]]
                print c1_set, c2_set
            except KeyError, e:
        ...
        #kdrew: if base species is not specified, input accs are the same as specified in the pdb
            ...
            c1_set = self.acc2chain[acc1]
            c2_set = self.acc2chain[acc2]

        print c1_set, c2_set

        return self.surface_area_set(c1_set,c2_set)
