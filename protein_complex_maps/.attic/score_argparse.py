...
#    np.savetxt(fileout, corr, delimiter='\t')
    ...
    parser = argparse.ArgumentParser(description='Get correlations pairwise correlations rows of elution profiles')
    parser.add_argument('infile', action="store", type=str, help="Tab separated matrix of IDs and fractions")
    parser.add_argument('outfile', action="store", nargs='?')
    parser.add_argument('method', action="store", type=str, help="poisson|cosine_poisson|euclidian_poisson|euclidian")
    parser.add_argument('iterations', action="store", type=int, default="1000", help="Number of iterations, 100-1000 is a good range")
    args = parser.parse_args()
    if args.outfile is None:
        args.outfile = args.infile + ".corr" + args.method
    print args 
    elut = el.load_elution(infile)
    if args.method=="poisson":
        corr = traver_corr(args.infile, repeat=args.iterations, metric=args.method)
    elif args.method == 'cosine_poisson':
        corr = poisson_repeat(infile, repeat=args.iterations, metric="cosine")  
    elif args.method == 'euclidean_poisson':
        corr = poisson_repeat(infile, repeat=args.iterations, metric="euclidean")  
    elif args.method=="euclidean":
        corr = pdist_score(infile, norm_rows=True, norm_cols=True, metric=args.method)

    np.savetext(outfile, corr, delimeter='\t')
