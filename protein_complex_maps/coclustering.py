...
def main():
	...
	parser.add_argument("--numOfClusters", action="store", dest="numOfClusters", required=True,
	...
	biclusters = evaluateClusters(msds, data_set, new_id_map, fit_model, args.plot_filename)
...
def evaluateClusters(msds, data_set, new_id_map, fit_model, plot_filename=None):
	...
	for j, row in enumerate(fit_model.rows_):
		...
		if len(bicluster_ids) > 1 and np.any(fit_model.columns_[j]):
			...
			bicluster_list.append((bicluster_ids, score))
