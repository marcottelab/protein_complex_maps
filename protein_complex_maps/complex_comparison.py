...
def main():
    ...
    print "Sensitivity,%s,%s" % (cplx_compare.sensitivity(),args.cluster_filename)
    print "PPV,%s,%s" % (cplx_compare.ppv(),args.cluster_filename)
    print "ACC,%s,%s" % (cplx_compare.acc(),args.cluster_filename)
    print "MMR,%s,%s" % (cplx_compare.mmr(),args.cluster_filename)
    print "PWMMR,%s,%s" % (cplx_compare.pwmmr(),args.cluster_filename)
    print "MMR_PWMMR_hmean,%s,%s" % (cplx_compare.mmr_pwmmr_hmean(),args.cluster_filename)
    print "Precision measure,%s,%s" % (cplx_compare.precision_measure(),args.cluster_filename)
    print "Recall measure,%s,%s" % (cplx_compare.recall_measure(),args.cluster_filename)
    print "Precision Recall product,%s,%s" % (cplx_compare.precision_recall_product(),args.cluster_filename)
    ccmm = cplx_compare.clique_comparison_metric_mean()
    print "Clique Precision Mean,%s,%s" % (ccmm['precision_mean'],args.cluster_filename)
    print  "Recall Mean,%s,%s" % (ccmm['recall_mean'],args.cluster_filename)
    ccmm = cplx_compare.clique_comparison_metric_mean(weighted=True)
    print "Clique Weighted Precision Mean,%s,%s" % (ccmm['precision_mean'],args.cluster_filename)
    print  "Weighted Recall Mean,%s,%s" % (ccmm['recall_mean'],args.cluster_filename)
    print "Clique Weighted hmean,%s,%s" % (hmean([ccmm['precision_mean'],ccmm['recall_mean']]),args.cluster_filename)
