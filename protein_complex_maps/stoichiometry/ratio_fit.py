		...
		for i, prot_ids in enumerate(complex_list):
			if msds_id_set >= set(prot_ids):                                                                              
				...
				if args.numOfProcs > 1:
					pool.apply_async(fit_ratios, (msds, prot_ids), dict(total_occupancy=args.total_occupancy, log_ratio=args.log_ratio, data_threshold=int(args.threshold)))
				else:
					fit_ratios(msds, prot_ids, total_occupancy=args.total_occupancy, log_ratio=args.log_ratio, data_threshold=int(args.threshold))

				#print "sent job"

	elif args.proteins != None:
		fit_ratios(msds, args.proteins, total_occupancy=args.total_occupancy, log_ratio=args.log_ratio, data_threshold=int(args.threshold), plot_filename=args.plot_filename)
	elif args.complexes != None:
		fit_ratios(msds, args.complexes, total_occupancy=args.total_occupancy, log_ratio=args.log_ratio, data_threshold=int(args.threshold), complexes_mode=True, plot_filename=args.plot_filename)
...
def fit_ratios(msds, ids, total_occupancy=False, log_ratio=False, data_threshold=10, sql_store=True, complexes_mode=False, plot_filename=None):
	...
	for i, pair in enumerate(ratio_dict):
		...
		print "Full set:  %s : %s, mean: %s (%s), median: %s (%s), std: %s (%s), counts: %s" % (pair[0], pair[1], ratio_dict_pair_array.mean(), np.exp(ratio_dict_pair_array.mean()), np.median(ratio_dict_pair_array), np.exp(np.median(ratio_dict_pair_array)), np.std(ratio_dict_pair_array), np.std(np.exp(ratio_dict_pair_array)), len(ratio_dict_pair_array))
		...
		for j, (mean, color) in enumerate(zip(clf.means_, color_iter)):
			...
			if len(class_data) > data_threshold:
				...
				#data_subplots[i].hist(class_data, color=color, bins=30)
				#data_subplots[i].plot(xs, mlab.normpdf(xs,mean,np.sqrt(clf._get_covars()[j][0])), color=color)
				data_subplots[i].set_title("%s : %s, converged? %s" % (pair[0], pair[1], clf.converged_, ))
		...
		data_subplots[i].plot(xs, density(xs))

		ax2 = data_subplots[i].twinx()
		#ax2.plot(xs, pdf, '-k')
		ax2.plot(xs, pdf_individual, '--k')



	data_subplots[-1].plot(N, AIC, '-k', label='AIC')
	data_subplots[-1].plot(N, BIC, '--k', label='BIC')
	data_subplots[-1].set_xlabel('n. components')
	data_subplots[-1].set_ylabel('information criterion')
	data_subplots[-1].legend(loc=2)
