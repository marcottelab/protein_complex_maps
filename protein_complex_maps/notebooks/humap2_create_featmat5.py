...
orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_df = pd.read_csv("/project/kdrew/data/protein_complex_maps/complex_map2/orig9k_bioplex2_hygeo_bioid_hygeo_boldt_apms_hygeo.featmat")
orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_df = orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_df.set_index("Unnamed: 0")

#kdrew: read in hypergeometric features
treiber_hygeo_gt4_df = pd.read_csv("/stor/work/Marcotte/project/kdrew/data/Treiber_etal/t_treiber_240212_gt4_prey_pairs_pvalcorr_logchoose.feat")

#kdrew: fill nans to be high scores, hit machine precision
treiber_hygeo_gt4_df['neg_ln_pval'] = treiber_hygeo_gt4_df['neg_ln_pval'].fillna(1000.0)
...
treiber_hygeo_gt4_gb_df = treiber_hygeo_gt4_df.groupby(treiber_hygeo_gt4_df.index).first()

#kdrew: join treiber hypergeometric features with full feature matrix
orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_df = orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_df.join(treiber_hygeo_gt4_gb_df, how="outer", rsuffix="_treiber_hygeo_gt4")

#kdrew: write out full feature matrix
orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_df.to_csv("/project/kdrew/data/protein_complex_maps/complex_map2/orig9k_bioplex2_hygeo_bioid_hygeo_boldt_apms_hygeo_treiber_hygeo.featmat")

#kdrew: names of columns to keep
...
#kdrew: append names of new treiber columns
cols.append('neg_ln_pval_treiber_hygeo_gt4')
cols.append('pair_count_treiber_hygeo_gt4')

#kdrew: feature matrix with only feature columns  
orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_trim_df = orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_df[cols]

#kdrew: create list of geneids from index
geneids = [eval(x)[0] for x in orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_trim_df.index]
geneids2 = [eval(x)[1] for x in orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_trim_df.index]

#kdrew: add column for ids 
orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_trim_df['id1'] = geneids
orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_trim_df['id2'] = geneids2

#kdrew: rearrange columns so ids are first
colswids = orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_trim_df.columns
orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_trim_df = orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_trim_df[list(colswids)[-2:] + list(colswids)[:-2]]

#kdrew: write out feature matrix
orig9k_bioplex2_hygeo_bioid_hygeo_boldt_hygeo_treiber_hygeo_trim_df.to_csv("/stor/work/Marcotte/project/kdrew/data/protein_complex_maps/complex_map2/orig9k_bioplex2_hygeo_bioid_hygeo_boldt_apms_hygeo_treiber_hygeo_trimCols.featmat",index=False)
