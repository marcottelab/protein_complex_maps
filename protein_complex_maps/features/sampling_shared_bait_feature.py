...
def hypergeo(feature_shared_bait_table, bh_correct=False):

    #kdrew: number of times pair is found (unique baits), 'k' in Hart etal 2007 
    print(feature_shared_bait_table)
    print(type(feature_shared_bait_table))
    print(feature_shared_bait_table.shape)
    ...
    #CDM question, what about gene_id2_str?  
    #CDM Hart et al 2007, m & n are the total number of interactions for proteins a and b
    ...
    output_dict = dict()
    output_dict['bait_id_column_str'] = []
    ...
    print("GOT HERE ", timestr)
    output_df = pd.DataFrame.from_records(output_dict)
    filename = timestr + "_" + "min2parallel"
    ...
    for gene_ids_str in ks.index:
        ...
        if len(gene_ids) == 2:
            ...
            try:
               ...
               outfile.write("%s,%s,%s, %s, %s\n" % (gene_ids, k, m, n, neg_ln_p))

               #print("%s k:%s n:%s m:%s -ln(p):%s" % (gene_ids, k, m, n, neg_ln_p))

               
               #output_dict['bait_id_column_str'].append(gene_ids_str)
...
def shared_bait_feature(feature_table, bait_id_column, id_column, bh_correct=False):
   ...
   #feature_table = feature_table.drop(bait_id_column)
    ...
    #Unindexed merge = slow. Delete this line later CDM
    #feature_shared_bait_table = feature_table.merge(feature_table, on='bait_id_column_str')

    logger.info(feature_shared_bait_table.head)
    ...
    logger.info(feature_shared_bait_table.head)
    logger.info(feature_shared_bait_table.columns.values)
    ...
    #cdm This is the point to do sampling
    ...
    for s in range(0,20):
       ...
       sample_size = int(0.3 * len(feature_shared_bait_table.bait_id_column_str.unique()))
       logger.info("sample_size: ", str(sample_size))
       
       selected_names = np.random.choice(feature_shared_bait_table.bait_id_column_str.unique(), sample_size, replace=False)
       sub_table = feature_shared_bait_table[feature_shared_bait_table.bait_id_column_str.isin(selected_names)]
       print(type(sub_table))
       logger.info(sub_table.head)
       sub_table2 = sub_table.drop_duplicates()
       sub_tables.append(sub_table2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--logname", action="store", dest="logname", required=False, default='shared_bait_feature.log',
                                    help="A filename for logged output")
    ...
    sample_dfs = shared_bait_feature(feature_table, args.bait_id_column, args.id_column, args.bh_correct)


    print(sample_dfs)
    pool = Pool(processes=5)
    pool.map(hypergeo, sample_dfs)
    sys.exit()
    logger.info("number of trials")
    #logger.info(len(list_of_dicts))
    ...
    for f in list_of_dfs:
        ...
        concat_df = concat_df.append(f)
    concat_df.to_csv("temp_min2_concatdf_sampled", index=False) 
    

    #concat_df = pd.DataFrame.from_records(list_of_dicts)
    ...
    #for i, d in enumerate(sample_dfs):
    #      d['group'] = i
